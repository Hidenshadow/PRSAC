...
$MediumOutRoot = Join-Path $Root "runs\sac_modified\ldac_v2_medium_seed2_4_20260623"
$HardOutRoot = Join-Path $Root "runs\sac_modified\ldac_v2_hard_seed2_4_20260623"
$MaxParallel = 4

function Get-CpuAffinityMask {
    $logical = (Get-CimInstance Win32_ComputerSystem).NumberOfLogicalProcessors
    # Leave headroom for Windows and monitoring processes so total CPU stays near 80%.
...
$Experiments = @(
    @{ Level = "level1_medium"; Config = "configs\levels\ppo_difficulty\level1_medium.json"; EvalEpisodes = 300; OutRoot = $MediumOutRoot },
    @{ Level = "level2_medium"; Config = "configs\levels\ppo_difficulty\level2_medium.json"; EvalEpisodes = 128; OutRoot = $MediumOutRoot },
    @{ Level = "level3_medium"; Config = "configs\levels\ppo_difficulty\level3_medium.json"; EvalEpisodes = 128; OutRoot = $MediumOutRoot },
    @{ Level = "level1_hard"; Config = "configs\levels\ppo_difficulty\level1_hard.json"; EvalEpisodes = 300; OutRoot = $HardOutRoot },
    @{ Level = "level2_hard"; Config = "configs\levels\ppo_difficulty\level2_hard.json"; EvalEpisodes = 128; OutRoot = $HardOutRoot },
    @{ Level = "level3_hard"; Config = "configs\levels\ppo_difficulty\level3_hard.json"; EvalEpisodes = 128; OutRoot = $HardOutRoot }
)

foreach ($rootPath in @($MediumOutRoot, $HardOutRoot)) {
...
foreach ($exp in $Experiments) {
    foreach ($seed in 2, 3, 4) {
        $jobOut = Join-Path $exp.OutRoot (Join-Path $exp.Level ("seed" + $seed))
        $allJobs += @{
            Level = $exp.Level
            Config = $exp.Config
...
foreach ($job in $allJobs) {
    $activeProcess = Get-CimInstance Win32_Process -Filter "Name='python.exe'" |
        Where-Object {
            $_.CommandLine -like "*run_shock_recovery_experiment.py*" -and
            $_.CommandLine -like ("*" + [string]$job.Output + "*")
        } |
    ...
    if ($activeProcess -ne $null) {
        ...
        if ($proc -ne $null) {
            ...
            Write-Host ("Resuming active {0} seed{1}: PID={2}" -f $job.Level, $job.Seed, $proc.Id)
        }
    }
}
...
foreach ($job in $allJobs) {
    $doneCheckpoint = Join-Path $job.Output "checkpoints\checkpoint_recovery_step_20480.pt"
    if (Test-Path $doneCheckpoint) {
        Write-Host ("Skipping existing completed run: {0} seed{1}" -f $job.Level, $job.Seed)
        continue
    }
    $alreadyRunning = $false
    foreach ($item in $running) {
        if ([string]$item.Output -eq [string]$job.Output) {
            $alreadyRunning = $true
            break
        }
    }
...
while ($queue.Count -gt 0 -or $running.Count -gt 0) {
    while ($queue.Count -gt 0 -and $running.Count -lt $MaxParallel) {
        $job = $queue.Dequeue()
        $logRoot = Join-Path $job.OutRoot "logs"
...
Write-Host ("All LDAC-SAC v2 medium/hard seed2/3/4 experiments finished.")
Write-Host ("Medium output: {0}" -f $MediumOutRoot)
Write-Host ("Hard output: {0}" -f $HardOutRoot)
