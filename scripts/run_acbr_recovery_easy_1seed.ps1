param(
    ...
    [int]$MaxParallel = 3,
    [string]$OutputBase = "runs/acbr_recovery_easy_1seed_matrix",
    ...
    [double]$AcbrCriticCoef = 0.50,
    [double]$AcbrQueryFraction = 0.50,
    [int]$AcbrQueryInterval = 4,
    [int]$AcbrNumCandidates = 24,
    [int]$AcbrNumRandomCandidates = 6,
    [int]$AcbrNumStructuredCandidates = 10,
    [double]$AcbrLocalSigma = 0.12,
    [double]$AcbrRiskLocalSigma = 0.20,
    [double]$AcbrUncertaintyCoef = 0.25,
    [double]$AcbrAnchorPenalty = 0.15,
    [double]$AcbrPolicyPenalty = 0.05,
...
$ManifestPath = Join-Path $ResolvedOutputBase "acbr_easy_launch.json"
...
function Start-Experiment {
    ...
    $Stdout = Join-Path $LogDir "$($Case.level)_$($Case.difficulty)_acbr_seed$($Case.seed).out.log"
    $Stderr = Join-Path $LogDir "$($Case.level)_$($Case.difficulty)_acbr_seed$($Case.seed).err.log"
    ...
    $Arguments = @(
        ...
        "--acbr-recovery-critic-coef", [string]$AcbrCriticCoef,
        "--acbr-recovery-query-fraction", [string]$AcbrQueryFraction,
        "--acbr-recovery-query-interval", [string]$AcbrQueryInterval,
        "--acbr-recovery-num-candidates", [string]$AcbrNumCandidates,
        "--acbr-recovery-num-random-candidates", [string]$AcbrNumRandomCandidates,
        "--acbr-recovery-num-structured-candidates", [string]$AcbrNumStructuredCandidates,
        "--acbr-recovery-local-sigma", [string]$AcbrLocalSigma,
        "--acbr-recovery-risk-local-sigma", [string]$AcbrRiskLocalSigma,
        "--acbr-recovery-uncertainty-coef", [string]$AcbrUncertaintyCoef,
        "--acbr-recovery-anchor-penalty", [string]$AcbrAnchorPenalty,
        "--acbr-recovery-policy-penalty", [string]$AcbrPolicyPenalty
    )
    if ($CleanOutput) {
...
Write-Host "ACBR recovery easy matrix queued: $($Pending.Count) runs; max_parallel=$MaxParallel"
...
Write-Host "ACBR recovery easy matrix finished. completed=$(@($AllLaunches | Where-Object { $_.status -eq 'completed' }).Count) failed=$($Failed.Count)"
