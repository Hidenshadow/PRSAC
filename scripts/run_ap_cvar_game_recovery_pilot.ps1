param(
    [int]$Seed = 0,
    [string]$OutputBase = "runs/ap_cvar_game_recovery_pilot",
...
foreach ($Case in $Cases) {
    ...
    $Stdout = Join-Path $LogDir "${Level}_${Difficulty}_ap_cvar_seed${Seed}.out.log"
    $Stderr = Join-Path $LogDir "${Level}_${Difficulty}_ap_cvar_seed${Seed}.err.log"
...
$ManifestPath = Join-Path $ResolvedOutputBase "ap_cvar_pilot_seed${Seed}_launch.json"
