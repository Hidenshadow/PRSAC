...
LEVEL_CONFIG="${LEVEL_CONFIG:-configs/levels/ppo_difficulty/level1_${DIFFICULTY}.json}"
OUTPUT_BASE="${OUTPUT_BASE:-runs/rl_baselines}"
SEEDS="${SEEDS:-0 1 2 3 4}"
SEED_COUNT="$(printf '%s\n' $SEEDS | wc -l | tr -d ' ')"
OUTPUT_ROOT="${OUTPUT_ROOT:-${OUTPUT_BASE}/${ALGO}/level1_${DIFFICULTY}_shock_recovery_${SEED_COUNT}seeds}"
...
HELDOUT_SEED_DEFAULT="1919"
HELDOUT_SEED="${HELDOUT_SEED:-$HELDOUT_SEED_DEFAULT}"
...
for seed in $SEEDS; do
  ...
  echo "=== Level 1 $ALGO shock-recovery difficulty=$DIFFICULTY seed=$seed seeds=$SEED_COUNT ==="
  "${args[@]}"
done
