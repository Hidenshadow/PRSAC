...
$cases = @(
  @{ Algo = "ppo"; Scenario = "level3_hard"; LevelConfig = "configs/levels/ppo_difficulty/level3_hard.json" },
  @{ Algo = "sac"; Scenario = "level3_hard"; LevelConfig = "configs/levels/ppo_difficulty/level3_hard.json" }
)
$cases = @($cases | Where-Object { $Algos -contains $_.Algo })
if ($cases.Count -eq 0) {
  throw "No cases selected. Use -Algos ppo, sac, or both."
}
...
function Test-SeedComplete {
  ...
  return (
    ...
    (Test-Path (Join-Path $SeedDir "checkpoints\checkpoint_recovery_step_$RecoveryTimesteps.pt"))
  )
}
...
function Start-BaselineSeed {
  ...
  $logPrefix = "level3_hard_refresh_$($Case.Algo)_seed$Seed"
...
foreach ($case in $cases) {
  foreach ($seed in $Seeds) {
    ...
    if (-not $Force -and (Test-SeedComplete -SeedDir $seedDir)) {
      Write-Host "Skipping complete $($case.Algo) $($case.Scenario) seed$seed. Use -Force to refresh with the current config."
      continue
    }
...
$summaryPath = Join-Path $outputRootPath "level3_hard_refresh_seed_process_summary.csv"
$completed | Export-Csv -Path $summaryPath -NoTypeInformation
$failed = @($completed | Where-Object { $_.ExitCode -ne 0 })
if ($failed.Count -gt 0) {
  Write-Error "One or more level3_hard refresh runs failed. See $summaryPath"
}
