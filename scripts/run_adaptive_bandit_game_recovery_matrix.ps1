param(
    ...
    [int]$MaxParallel = 6,
    [string]$OutputBase = "runs/adaptive_bandit_game_recovery_matrix",
    ...
    [double]$GameBanditPriorMix = 0.10,
...
$ManifestPath = Join-Path $ResolvedOutputBase "adaptive_bandit_matrix_launch.json"
...
function Start-Experiment {
    ...
    $Stdout = Join-Path $LogDir "$($Case.level)_$($Case.difficulty)_adaptive_bandit_seed$($Case.seed).out.log"
    $Stderr = Join-Path $LogDir "$($Case.level)_$($Case.difficulty)_adaptive_bandit_seed$($Case.seed).err.log"
...
Write-Host "Adaptive-bandit matrix queued: $($Pending.Count) runs; max_parallel=$MaxParallel"
...
Write-Host "Adaptive-bandit matrix finished. completed=$(@($AllLaunches | Where-Object { $_.status -eq 'completed' }).Count) failed=$($Failed.Count)"
