...
LEVEL_CONFIG="${LEVEL_CONFIG:-configs/levels/ppo_difficulty/level3_${DIFFICULTY}.json}"
BASE_CONFIG="${BASE_CONFIG:-configs/ppo_lunar_viper_relative_reward.json}"
OUTPUT_BASE="${OUTPUT_BASE:-runs/rl_baselines}"
SEEDS="${SEEDS:-0 1 2 3 4}"
SEED_COUNT="$(printf '%s\n' $SEEDS | wc -l | tr -d ' ')"
OUTPUT_ROOT="${OUTPUT_ROOT:-${OUTPUT_BASE}/${ALGO}/level3_${DIFFICULTY}_shock_recovery_${SEED_COUNT}seeds}"
...
for seed in $SEEDS; do
  ...
  echo "=== Level 3 $ALGO shock-recovery difficulty=$DIFFICULTY seed=$seed seeds=$SEED_COUNT ==="
  "${args[@]}"
done
