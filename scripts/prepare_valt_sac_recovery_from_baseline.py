#!/usr/bin/env python
"""Prepare VALT-SAC recovery-only runs from existing vanilla SAC nominal checkpoints."""
...
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "runs" / "sac_modified" / "valt_sac_from_sac_nominal_9scenarios_2seeds"
...
VALT_SAC_RECOVERY_DEFAULTS = {
    ...
    "valt_sac_recovery_enabled": True,
    "valt_sac_eps_start": 0.0,
    "valt_sac_eps_end": 0.08,
    "valt_sac_kappa_start": 0.0,
    "valt_sac_kappa_end": 0.30,
    "valt_sac_schedule_steps": 20_480,
    "valt_sac_schedule": "linear",
    "valt_sac_bound_iters": 2,
    "valt_sac_worst_step_size": 0.0,
    "valt_sac_sgld_noise": 0.0,
    "valt_sac_policy_reg_coef": 1.0,
    "valt_sac_random_start": True,
    "valt_sac_clip_low": 0.0,
    "valt_sac_clip_high": 1.0,
    "valt_sac_attack_deterministic": False,
}
...
def prepare_one(
    ...
    command_args.update(VALT_SAC_RECOVERY_DEFAULTS)
    command_args.update(
        {
            "algo": "sac",
    ...
    run_config["protocol"] = "valt_sac_recovery_only_from_vanilla_sac_nominal"
    run_config["algorithm"] = "valt_sac_recovery"
    ...
    run_config["valt_sac_recovery_config"] = dict(VALT_SAC_RECOVERY_DEFAULTS)
...
def main() -> int:
    ...
    manifest = {
        ...
        "valt_sac_recovery_config": dict(VALT_SAC_RECOVERY_DEFAULTS),
    }
    output_root.mkdir(parents=True, exist_ok=True)
    manifest_path = output_root / "prepare_manifest.json"
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    print(f"Prepared {len(prepared)} VALT-SAC recovery seed dirs under {output_root}")
