param(
    ...
    [int]$MaxParallel = 9,
    [string]$OutputBase = "runs/bagr_recovery_seed0_matrix",
    ...
    [double]$BagrAuxCoef = 0.10,
    [int]$BagrLatentDim = 64,
    [int]$BagrHiddenDim = 64,
    [int]$BagrMaxAttackVariants = 6,
    [double]$BagrBeliefTemperature = 0.25,
    [double]$BagrBeliefPriorMix = 0.10,
    [double]$BagrResidualL2Coef = 0.10,
    [double]$BagrResidualBarrierCoef = 2.0,
    [double]$BagrResidualActionLimit = 0.18,
    [double]$BagrConfidenceLimitScale = 1.0,
...
$ManifestPath = Join-Path $ResolvedOutputBase "bagr_matrix_launch.json"
...
function Start-Experiment {
    ...
    $Stdout = Join-Path $LogDir "$($Case.level)_$($Case.difficulty)_bagr_seed$($Case.seed).out.log"
    $Stderr = Join-Path $LogDir "$($Case.level)_$($Case.difficulty)_bagr_seed$($Case.seed).err.log"
    ...
    $Arguments = @(
        ...
        "--bagr-recovery-aux-coef", [string]$BagrAuxCoef,
        "--bagr-recovery-latent-dim", [string]$BagrLatentDim,
        "--bagr-recovery-hidden-dim", [string]$BagrHiddenDim,
        "--bagr-recovery-max-attack-variants", [string]$BagrMaxAttackVariants,
        "--bagr-recovery-belief-temperature", [string]$BagrBeliefTemperature,
        "--bagr-recovery-belief-prior-mix", [string]$BagrBeliefPriorMix,
        "--bagr-recovery-residual-l2-coef", [string]$BagrResidualL2Coef,
        "--bagr-recovery-residual-barrier-coef", [string]$BagrResidualBarrierCoef,
        "--bagr-recovery-residual-action-limit", [string]$BagrResidualActionLimit,
        "--bagr-recovery-confidence-limit-scale", [string]$BagrConfidenceLimitScale
    )
    if ($CleanOutput) {
...
Write-Host "BAGR recovery matrix queued: $($Pending.Count) runs; max_parallel=$MaxParallel"
...
Write-Host "BAGR recovery matrix finished. completed=$(@($AllLaunches | Where-Object { $_.status -eq 'completed' }).Count) failed=$($Failed.Count)"
