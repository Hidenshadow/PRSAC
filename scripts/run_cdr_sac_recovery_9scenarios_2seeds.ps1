param(
    [int[]]$Seeds = @(0, 1),
    [string]$OutputName = "cdr_sac_from_sac_nominal_9scenarios_2seeds_20260627",
    [int]$MaxParallel = 2,
...
$PrepareScript = Join-Path $ScriptDir "prepare_cdr_sac_recovery_from_baseline.py"
...
Write-Host ("Preparing CDR-SAC recovery dirs under {0}" -f $OutputRoot)
& $Python @prepareArgs
...
foreach ($scenario in $Scenarios) {
    foreach ($seed in $Seeds) {
        ...
        if (Test-CompleteSeedDir -SeedDir $SeedDir) {
            Write-Host ("Skipping complete CDR-SAC {0} seed{1}: {2}" -f $scenario, $seed, $SeedDir)
            continue
        }
...
Write-Host ("Queued {0} incomplete CDR-SAC recovery jobs with MaxParallel={1}" -f $Jobs.Count, $MaxParallel)
if ($DryRun) {
    foreach ($job in $Jobs) {
...
function Start-NextJobs {
    while (($Running.Count -lt $MaxParallel) -and ($Pending.Count -gt 0)) {
        ...
        Write-Host ("Started CDR-SAC {0} seed{1}: PID={2}" -f $job.Scenario, $job.Seed, $proc.Id)
    }
}
...
while ($Running.Count -gt 0) {
    ...
    foreach ($item in $Running) {
        if ($item.Process.HasExited) {
            ...
            Write-Host ("Finished CDR-SAC {0} seed{1}: exit={2} complete={3} duration={4:N1}min" -f $item.Scenario, $item.Seed, $exitCode, $complete, $durationMin)
...
$SummaryPath = Join-Path $OutputRoot ("cdr_sac_recovery_run_summary_{0}.csv" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
...
Write-Host ("All CDR-SAC recovery jobs complete: {0}" -f $OutputRoot)
