param(
    ...
    [string]$OutputName = "valt_sac_from_sac_nominal_9scenarios_2seeds_20260628",
    [int]$MaxParallel = 2,
...
$PrepareScript = Join-Path $ScriptDir "prepare_valt_sac_recovery_from_baseline.py"
...
$SeedInts = @(Convert-Seeds -RawSeeds $Seeds)
if ($SeedInts.Count -eq 0) {
    throw "No seeds requested."
}
if ($Scenarios.Count -eq 0) {
    throw "No scenarios requested."
}
...
Write-Host ("Preparing VALT-SAC recovery dirs under {0}" -f $OutputRoot)
& $Python @prepareArgs
...
foreach ($scenario in $Scenarios) {
    foreach ($seed in $SeedInts) {
        ...
        if (Test-CompleteSeedDir -SeedDir $SeedDir) {
            Write-Host ("Skipping complete VALT-SAC {0} seed{1}: {2}" -f $scenario, $seed, $SeedDir)
            continue
        }
...
Write-Host ("Queued {0} incomplete VALT-SAC recovery jobs with MaxParallel={1}" -f $Jobs.Count, $MaxParallel)
if ($DryRun) {
    foreach ($job in $Jobs) {
...
function Start-NextJobs {
    while (($Running.Count -lt $MaxParallel) -and ($Pending.Count -gt 0)) {
        ...
        Write-Host ("Started VALT-SAC {0} seed{1}: PID={2}" -f $job.Scenario, $job.Seed, $proc.Id)
    }
}
...
while ($Running.Count -gt 0) {
    ...
    foreach ($item in $Running) {
        if ($item.Process.HasExited) {
            ...
            Write-Host ("Finished VALT-SAC {0} seed{1}: exit={2} complete={3} duration={4:N1}min" -f $item.Scenario, $item.Seed, $exitCode, $complete, $durationMin)
...
$SummaryPath = Join-Path $OutputRoot ("valt_sac_recovery_run_summary_{0}.csv" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
...
Write-Host ("All VALT-SAC recovery jobs complete: {0}" -f $OutputRoot)
