param(
    ...
    [int]$MaxParallel = 6,
    [string]$OutputBase = "runs/pr_ap_cvar_game_recovery_matrix",
    ...
    [double]$PrAlpha = 0.50,
    [string]$PrAuxLossType = "cpa",
    [double]$PrQueryFraction = 0.25,
    [int]$PrQueryInterval = 4,
    [int]$PrNumCandidates = 24,
    [int]$PrNumRandomCandidates = 8,
    [double]$PrRegretWeightMax = 3.0,
...
$ManifestPath = Join-Path $ResolvedOutputBase "pr_ap_cvar_matrix_launch.json"
...
function Start-Experiment {
    ...
    $Stdout = Join-Path $LogDir "$($Case.level)_$($Case.difficulty)_pr_ap_cvar_seed$($Case.seed).out.log"
    $Stderr = Join-Path $LogDir "$($Case.level)_$($Case.difficulty)_pr_ap_cvar_seed$($Case.seed).err.log"
    ...
    $Arguments = @(
        ...
        "--pr-recovery-alpha", [string]$PrAlpha,
        "--pr-recovery-aux-loss-type", $PrAuxLossType,
        "--pr-recovery-query-fraction", [string]$PrQueryFraction,
        "--pr-recovery-query-interval", [string]$PrQueryInterval,
        "--pr-recovery-num-candidates", [string]$PrNumCandidates,
        "--pr-recovery-num-random-candidates", [string]$PrNumRandomCandidates,
        "--pr-recovery-regret-weight-max", [string]$PrRegretWeightMax
    )
    if ($CleanOutput) {
...
Write-Host "PR-AP-CVaR matrix queued: $($Pending.Count) runs; max_parallel=$MaxParallel"
...
Write-Host "PR-AP-CVaR matrix finished. completed=$(@($AllLaunches | Where-Object { $_.status -eq 'completed' }).Count) failed=$($Failed.Count)"
