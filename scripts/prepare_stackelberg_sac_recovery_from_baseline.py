#!/usr/bin/env python
"""Prepare Stackelberg-SAC recovery-only runs from existing vanilla SAC nominal checkpoints."""
...
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "runs" / "sac_modified" / "stackelberg_sac_from_sac_nominal_9scenarios_2seeds"
...
STACKELBERG_SAC_DEFAULTS = {
    "game_recovery_enabled": True,
    "sac_game_recovery_enabled": True,
    "cdr_recovery_enabled": False,
    "game_attack_mixture_size": 5,
    "game_attack_jitter": 0.15,
    "game_attack_sampler": "qre_minimax",
    "game_attack_variant_mode": "scale",
    "game_bandit_cost_key": "policy_cost",
    "game_bandit_eta": 0.8,
    "game_bandit_min_prob": 0.05,
    "game_bandit_prior_mix": 0.10,
    "game_bandit_benchmark_floor": 0.20,
    "qre_minimax_recovery_enabled": True,
    "qre_temperature_start": 2.0,
    "qre_temperature_end": 0.85,
    "qre_temperature_schedule": "exp",
    "qre_response_rate": 0.40,
    "qre_cost_ema_beta": 0.70,
    "qre_prior_mix": 0.15,
    "qre_min_prob": 0.05,
    "qre_max_prob_cap": 0.55,
    "qre_exploration_bonus": 0.15,
    "qre_benchmark_floor": 0.20,
    "qre_cost_normalization": "std",
    "sac_game_anchor_coef": 0.25,
    "sac_game_advantage_coef": 0.10,
    "sac_game_q_margin": 0.02,
    "sac_game_gate_temperature": 0.05,
    "sac_game_lambda_drift_coef": 0.0,
    "sac_game_lambda_drift_margin": 0.10,
    "sac_game_risk_drift_coef": 0.0,
    "sac_game_risk_drift_margin": 0.10,
    "sac_game_anchor_barrier_coef": 0.0,
    "sac_game_anchor_radius": 0.15,
    "sac_game_risk_action_indices": "energy,hazard,communication,illumination",
    "sac_recovery_deterministic_actor_update": False,
    "sac_recovery_target_entropy_scale": 1.0,
    "sac_recovery_fixed_alpha": None,
    "sac_recovery_rollout_deterministic_prob": 0.0,
    "sac_recovery_rollout_noise_std": 0.0,
    "sac_recovery_log_std_penalty_coef": 0.0,
    "sac_recovery_log_std_target": -1.5,
...
def prepare_one(
    ...
    command_args.update(STACKELBERG_SAC_DEFAULTS)
    command_args.update(
        {
            "algo": "sac",
    ...
    run_config["protocol"] = "stackelberg_sac_recovery_only_from_vanilla_sac_nominal"
    run_config["algorithm"] = "stackelberg_sac_recovery"
    ...
    run_config["stackelberg_sac_recovery_config"] = dict(STACKELBERG_SAC_DEFAULTS)
...
def main() -> int:
    ...
    manifest = {
        ...
        "stackelberg_sac_recovery_config": dict(STACKELBERG_SAC_DEFAULTS),
    }
    output_root.mkdir(parents=True, exist_ok=True)
    manifest_path = output_root / "prepare_manifest.json"
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    print(f"Prepared {len(prepared)} Stackelberg-SAC recovery seed dirs under {output_root}")
