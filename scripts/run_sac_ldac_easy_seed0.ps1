...
$OutRoot = Join-Path $Root "runs\sac_modified\ldac_easy_seed0_20260621"
...
while ($queue.Count -gt 0 -or $running.Count -gt 0) {
    while ($queue.Count -gt 0 -and $running.Count -lt $MaxParallel) {
        ...
        $argsList = @(
            ...
            "--sac-game-anchor-coef", "0.35",
            "--sac-game-advantage-coef", "0.15",
            "--sac-game-q-margin", "0.02",
            "--sac-game-gate-temperature", "0.05",
            "--sac-game-anchor-barrier-coef", "1.0",
            "--sac-game-anchor-radius", "0.12",
            "--sac-recovery-deterministic-actor-update",
            "--sac-recovery-fixed-alpha", "0.02",
            "--sac-recovery-target-entropy-scale", "0.10",
            "--sac-recovery-rollout-deterministic-prob", "0.75",
            "--sac-recovery-rollout-noise-std", "0.02"
        )
...
Write-Host ("All LDAC-SAC easy seed0 experiments finished: {0}" -f $OutRoot)
